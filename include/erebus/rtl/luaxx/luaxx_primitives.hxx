...
#include <erebus/rtl/luaxx/luaxx_exception_types.hxx>
#include <erebus/rtl/luaxx/luaxx_traits.hxx>
...
inline T& _get(_id<T&>, lua_State* l, const int index) 
{
    ...
    if (ptr == nullptr) 
    {
        throw TypeError{ std::source_location::current(), MetatableRegistry::GetTypeName(l, Er::userType<T>())};
    }
...
template <typename T>
inline typename std::enable_if<!is_primitive<typename std::decay<T>::type>::value, T>::type
...
template <typename T>
inline typename std::enable_if<!is_primitive<typename std::decay<T>::type>::value>::type
...
_push(lua_State* l, T&& t) 
{
    ...
    {
        throw CopyUnregisteredType(std::source_location::current(), typeid(t));
    }
...
inline void _push_dispatcher(lua_State* l, const std::tuple<T...>& values, _indices<N...>) 
{
...
inline void _push(lua_State* l, const std::tuple<T...> &values) 
{
    ...
    _push_dispatcher(l, values, typename _indices_builder<num_values>::type());
}

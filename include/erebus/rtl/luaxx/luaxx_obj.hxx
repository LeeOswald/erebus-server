...
#include <erebus/rtl/luaxx/luaxx_obj_fun.hxx>
...
private:
    ...
    template <typename M>
    void _register_member(lua_State* state, T* t, const char* member_name, M T::* member) 
    {
        _register_member(state, t, member_name, member, typename std::is_const<M>::type{});
    }

    template <typename M>
    void _register_member(lua_State* state, T* t, const char* member_name, M T::* member, std::false_type) 
    {
    ...
    template <typename M>
    void _register_member(lua_State* state, T* t, const char* member_name, M T::* member, std::true_type) 
    {
        std::function<M()> lambda_get = [t, member]() 
        {
            return t->*member;
    ...
    void _register_member(lua_State* state, T* t, const char* fun_name, Ret(T::*fun)(Args&&...)) 
    {
    ...
    void _register_members(lua_State* state, T* t, const char* name, M member, Ms... members) 
    {

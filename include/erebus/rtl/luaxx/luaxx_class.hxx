...
#include <erebus/rtl/luaxx/luaxx_class_fun.hxx>
#include <erebus/rtl/luaxx/luaxx_ctor.hxx>
#include <erebus/rtl/luaxx/luaxx_dtor.hxx>
#include <erebus/rtl/luaxx/luaxx_metatable_registry.hxx>
#include <erebus/rtl/luaxx/luaxx_util.hxx>
...
template <typename T, typename A, typename... Members>
class Class 
    : public BaseClass 
{
private:
    ...
    template <typename M>
    void _register_member(lua_State* state, const char* member_name, M T::* member) 
    {
        _register_member(state, member_name, member, typename std::is_const<M>::type{});
    }

    template <typename M>
    void _register_member(lua_State* state, const char* member_name, M T::* member, std::false_type) 
    {
    ...
    template <typename M>
    void _register_member(lua_State* state, const char* member_name, M T::* member, std::true_type) 
    {
        std::function<M(T*)> lambda_get = [member](T* t) 
        {
            return t->*member;
    ...
    void _register_member(lua_State* state, const char* fun_name, Ret(T::* fun)(Args&&...)) 
    {
    ...
    template <typename Ret, typename... Args>
    void _register_member(lua_State* state, const char* fun_name, Ret(T::* fun)(Args...) const) 
    {
        std::function<Ret(const T*, Args...)> lambda = [fun](const T* t, Args... args) 
        {
            return (t->*fun)(args...);
        };
    ...
    void _register_members(lua_State* state, const char* name, M member, Ms... members) 
    {
...
public:
    Class(lua_State* state, const std::string& name, Members... members) 
        : _name(name) 
    {

...
public:
    ...
    RegisteredType* findOrAdd(std::string_view key)
    {
        ...
        // add
        {
            ...
            {
                // really added
                Er::Log::debug(m_log, "Registered type [{}] -> {}", key, id);
            }
...
EREBUS_EXPORT void initializeTypeRegistry(Er::Log::ILog* log)
{
    ErAssert(!s_utr);
    s_utr = new UserTypeRegistry(log);
}

EREBUS_EXPORT void finalizeTypeRegistry() noexcept
{
    delete s_utr;
    s_utr = nullptr;
}

EREBUS_EXPORT RegisteredType* lookupType(std::string_view name)
{
    ErAssert(s_utr);
